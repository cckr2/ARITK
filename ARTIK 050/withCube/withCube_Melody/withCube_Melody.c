...
#define TOPIC 			"/v1.1/messages/d909f55c1f714ba2a89fd02959e81caa"
...
static void lis3dh_run(){
	...
	int readValue[3],maxValue[3];
	int dir, current_dir,real_dir,prev_dir,dir_count;
	int16_t temp;
	...
	while(1){
		...
		for(j=0;j<10;j++){
			...
			for(i=0;i<3;i++){
				...
				if(current_dir < readValue[i]){
					dir = i;
					real_dir = dir;
					if(readValue[i]<0){
						real_dir += 3;
					}
					current_dir = readValue[i];
				}
		...
		if(shock>175){
			if(subshock>180){
				...
				switch(dir){
				case 0:sprintf(message, msg, CUBE_TYPE, CUBE_NUM, "+z", shock);break;
				case 1:sprintf(message, msg, CUBE_TYPE, CUBE_NUM, "+y", shock);break;
				case 2:sprintf(message, msg, CUBE_TYPE, CUBE_NUM, "+x", shock);break;
				case 3:sprintf(message, msg, CUBE_TYPE, CUBE_NUM, "-z", shock);break;
				case 4:sprintf(message, msg, CUBE_TYPE, CUBE_NUM, "-y", shock);break;
				case 5:sprintf(message, msg, CUBE_TYPE, CUBE_NUM, "-x", shock);break;
				}
				wiced_rtos_delay_milliseconds( 10 );
...
static wiced_result_t connect()
{
    ...
    conninfo.username = "d909f55c1f714ba2a89fd02959e81caa";
    conninfo.password = "a43e247067574f75b0280ba6806b53c8";
