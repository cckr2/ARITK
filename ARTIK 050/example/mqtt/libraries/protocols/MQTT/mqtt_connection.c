 ...
 ******************************************************/
wiced_result_t mqtt_connection_init( const wiced_ip_address_t *address, uint16_t port_number, wiced_interface_t interface, const wiced_mqtt_callback_t callbacks, mqtt_connection_t *conn, const wiced_mqtt_security_t *security, wiced_bool_t onlyTLS )
{
    if ( port_number == 0 )
    {
    ...
    return mqtt_network_init( address, port_number, interface, conn, &conn->socket, security , onlyTLS);
}
